...
def LTC():
        cryptsy = json.load(urllib2.urlopen('http://pubapi.cryptsy.com/api.php?method=singlemarketdata&marketid=3'))
        #if cryptsy["success"] != "1":
        #                raise Exception("Cryptsy API failed!")
        current_price = cryptsy["return"]["markets"]["LTC"]["lasttradeprice"]
